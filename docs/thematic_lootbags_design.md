...
[70–99]    Электрика, КИПиА и автоматика (70: Скрутка проводов, 71: Сумка электрика, 72: Серверный блок)
           └ Резерв 73–99: компоненты высоковольтных подстанций, логические платы, датчики РБМК
[100–129]  Оружие, патроны и обвесы (100: Старый подсумок, 101: Патронный цинк, 102: Оружейный кофр)
...
#### 🖥️ [Meta 72] Серверный блок
* **Уровень:** 3 | **Rarity:** 2 | **Дроп:** `3 - 5` | `CombineTrashGroup="false"`
* *Лор:* Вычислительный блок из серверной стойки промышленной АСУ ТП.
* **Содержимое:**
  * `hbm:item.rbmk_tool` (**Консольный ключ РБМК**) | Кол-во: 1 | Вес: 35 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.crt_display` (**ЭЛТ-дисплей**) | Кол-во: 1 | Вес: 40 | `LimitedDropCount="1"` | `RandomAmount="false"`
  ...
  * `hbm:item.circuit:12` (**Сверхсложная схема**) | Кол-во: 1–2 | Вес: 25 | `RandomAmount="true"`
  * `OpenComputers:item:19` (**Магнитный металлический диск**) | Кол-во: 1–2 | Вес: 45 | `RandomAmount="true"`
  * `computronics:computronics.tape:2` (**Алмазная кассета данных**) | Кол-во: 1 | Вес: 30 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `ProjRed|Expansion:projectred.expansion.plan` (**Recipe Plan**) | Кол-во: 1 | Вес: 35 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.hard_drive` (**Жёсткий диск**) | Кол-во: 1 | Вес: 30 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.wire_fine:38` (**Сверхпроводящий провод**) | Кол-во: 2–4 | Вес: 35 | `RandomAmount="true"`
  * `hbm:item.circuit:16` (**Интерфейсная плата**) | Кол-во: 1 | Вес: 20 | `RandomAmount="false"`
  * `hbm:item.circuit:13` (**Вычислительный процессор**) | Кол-во: 1 | Вес: 25 | `RandomAmount="false"`
  * `hbm:item.circuit:6` (**Интегральная микросхема**) | Кол-во: 2–4 | Вес: 45 | `RandomAmount="true"`
  ...
  * `hbm:item.circuit:21` (**Атомные часы**) | Кол-во: 1 | Вес: 15 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.battery_pack:3` (**Энергетическая ячейка**) | Кол-во: 1 | Вес: 25 | `RandomAmount="false"`
  * `hbm:item.billet_silicon` (**Кремниевая заготовка**) | Кол-во: 4–8 | Вес: 50 | `RandomAmount="true"`
...
#### 🎒 [Meta 100] Старый подсумок
* **Уровень:** 1 | **Rarity:** 0 | **Дроп:** `1 - 2` | `CombineTrashGroup="true"`
* *Лор:* Брезентовый подсумок часового с боеприпасами, стрелами и взрывпакетами.
* **Механика ItemGroup:** Выпадение любого калиберного комплекта гарантированно выдаёт все входящие в него предметы в рамках одного ролла:
  * `pack_shotgun`: `hbm:item.pellet_buckshot` (**Картечь**, 12–24 шт.) + `hbm:item.casing:4` (**Пластиковая гильза 12k**, 8–16 шт.) | Вес: 100
  * `pack_9mm`: `hbm:item.ammo_standard:21` (**Патрон 9мм FMJ**, 16–32 шт.) + `hbm:item.casing:0` (**Малая гильза**, 8–16 шт.) | Вес: 90
  * `pack_scraps`: `hbm:item.debris_shrapnel` (**Шрапнель**, 10–20 шт.) + `hbm:item.powder_coal` (**Угольная пыль**, 6–12 шт.) | Вес: 50
* **Вторичное тактическое содержимое:**
  * `minecraft:gunpowder` (**Порох**) | Кол-во: 4–8 | Вес: 70 | `RandomAmount="true"`
  * `minecraft:iron_ingot` (**Железо**) | Кол-во: 2–4 | Вес: 65 | `RandomAmount="true"`
  * `minecraft:flint` (**Кремень**) | Кол-во: 2–4 | Вес: 60 | `RandomAmount="true"`
  * `minecraft:feather` (**Перья**) | Кол-во: 4–8 | Вес: 65 | `RandomAmount="true"`
  * `minecraft:arrow` (**Стрелы**) | Кол-во: 8–16 | Вес: 70 | `RandomAmount="true"`
  * `minecraft:bow` (**Лук**) | Кол-во: 1 | Вес: 40 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.stick_dynamite` (**Динамитная шашка**) | Кол-во: 2–4 | Вес: 45 | `RandomAmount="true"`
  * `hbm:item.grenade_lemon` (**Граната «Лимонка»**) | Кол-во: 1–2 | Вес: 30 | `RandomAmount="true"`
  * `hbm:item.ammo_standard:6` (**Обойма револьвера .357**) | Кол-во: 1 | Вес: 50 | `RandomAmount="false"`
  * `hbm:item.gun_kit_1` (**Оружейный набор I**) | Кол-во: 1 | Вес: 35 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.powder_lead` (**Свинцовый порошок**) | Кол-во: 4–8 | Вес: 60 | `RandomAmount="true"`
  * `hbm:item.rag` (**Тряпка**) | Кол-во: 2–4 | Вес: 75 | `RandomAmount="true"`

#### 🎖️ [Meta 101] Патронный цинк
* **Уровень:** 2 | **Rarity:** 1 | **Дроп:** `2 - 3` | `CombineTrashGroup="true"`
* *Лор:* Вскрытый оцинкованный ящик с автоматными патронами, бронебойными пулями и порохом.
* **Калиберные комплекты (`ItemGroup`):**
  * `pack_762`: `hbm:item.ammo_standard:29` (**7.62мм FMJ**, 16–32 шт.) + `hbm:item.ammo_standard:31` (**7.62мм AP**, 12 шт.) | Вес: 80
  * `pack_556`: `hbm:item.ammo_standard:25` (**5.56мм FMJ**, 16–32 шт.) + `hbm:item.casing:1` (**Малая стальная гильза**, 16 шт.) | Вес: 80
  * `pack_casings`: `hbm:item.casing:0` (**Малая бронзовая гильза**, 24 шт.) + `hbm:item.casing:2` (**Большая бронзовая гильза**, 16 шт.) | Вес: 50
* **Вторичные боеприпасы и снаряжение:**
  * `hbm:item.casing:3` (**Большая стальная гильза**) | Кол-во: 8–16 | Вес: 45 | `RandomAmount="true"`
  * `hbm:item.weapon_mod_special:2` (**Ножовка мастера**) | Кол-во: 1 | Вес: 30 | `LimitedDropCount="1"` | `RandomAmount="false"`. *Смысл:* Модификатор под обрез.
  * `hbm:item.casing_bag` (**Сумка гильзоулавливателя**) | Кол-во: 1 | Вес: 40 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.cordite` (**Кордит**) | Кол-во: 4–8 | Вес: 65 | `RandomAmount="true"`. *Смысл:* Бездымный порох.
  * `hbm:item.gun_kit_1` (**Оружейный набор I**) | Кол-во: 1 | Вес: 45 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.ammo_standard:22` (**Патрон 9мм AP**) | Кол-во: 12–24 | Вес: 55 | `RandomAmount="true"`
  * `hbm:item.ammo_standard:26` (**Патрон 5.56мм AP**) | Кол-во: 10–20 | Вес: 50 | `RandomAmount="true"`
  * `hbm:item.ammo_standard:23` (**Патрон 9мм HP**) | Кол-во: 12–24 | Вес: 50 | `RandomAmount="true"`
  * `hbm:item.ammo_standard:30` (**Патрон 7.62мм HP**) | Кол-во: 10–20 | Вес: 50 | `RandomAmount="true"`
  * `hbm:item.plate_steel` (**Стальная пластина**) | Кол-во: 2–4 | Вес: 60 | `RandomAmount="true"`
  * `hbm:item.plate_kevlar` (**Лист кевлара**) | Кол-во: 1–2 | Вес: 45 | `RandomAmount="true"`
  * `hbm:item.stealth_boy` (**Стелс-Бой**) | Кол-во: 1 | Вес: 30 | `LimitedDropCount="1"` | `RandomAmount="false"`
  * `hbm:item.ammo_standard:34` (**Патрон .50 BMG**) | Кол-во: 6–12 | Вес: 25 | `RandomAmount="true"`
  * `hbm:item.flame_opinion` (**Огнесмесь**) | Кол-во: 1–2 | Вес: 35 | `RandomAmount="true"`
...
| **`POOL_VAULT_LAB`** | Лабораторное хранилище | Инженерный ЗИП (42), Серверный блок (72), Кейс технолога (281), Банк семян (162) | 2 |
...
| **72** | Серверный блок | 3 | 2 | 3–5 | false | 20 |
| **100** | Старый подсумок | 1 | 0 | 1–2 | true | 18 |
| **101** | Патронный цинк | 2 | 1 | 2–3 | true | 20 |
