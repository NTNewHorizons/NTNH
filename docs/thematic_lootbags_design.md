...
3. **Трёхступенчатая шкала сохранности и объёма дропа:**
   ...
   * 🥉 **Tier 1: Потрёпанный / Вскрытый** (`2–4 типа/слота предметов`, отдельные профили — `1–3` или `2–3`). Найден на поверхности и в открытых заброшках. Содержит умеренную примесь мусора группы 0 (`CombineTrashGroup="false"`). Механика зачарования на Удачу отключена (`AllowFortuneBags=false`) для сохранения баланса.
   * 🥈 **Tier 2: Штатный / Производственный** (`2–4 типа/слота предметов`, расширенный — `3–5` слотов). Шкафчики заводских цехов, бытовки, лаборатории. Профильный инструмент, приборы и качественные расходники (`CombineTrashGroup="false"`).
...
* **Содержимое:**
  ...
  * `minecraft:web` (**Паутина**) | Кол-во: 1–2 | Вес: 8 | `RandomAmount="true"`. *Смысл:* Следы запустения / полевое кровоостанавливающее.
...
* **Содержимое:**
  ...
  * `minecraft:web` (**Паутина**) | Кол-во: 1–2 | Вес: 5 | `RandomAmount="true"`. *Смысл:* Паутина на старом планшете из пещеры.
