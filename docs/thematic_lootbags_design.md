...
### 1.5. Эргономика стаков, долговечность и реализм (Anti-Neuro-Slop)
...
2. **Эргономичный объём стаков:** Расходники должны выпадать в объёме, достаточном для практического применения (16–32 нитей, 12–24 удобрений, 8–16 лестниц, 4–8 брикетов), исключая раздражающие микро-стаки по 1–2 единицы.
...
* `LimitedDropCount="1"`: предмет может выпасть максимум 1 раз **на игрока за всё время игры в данном мире**. Мод Enhanced LootBags персистентно фиксирует выпадения по UUID игрока в файле `saves/<World>/LootBagStorage/LootBags.dat`. По достижении лимита предмет навсегда исключается из роллов для этого игрока. Используется для уникального инструмента, брони, приборов и квестовых диковинок, чтобы исключить захламление базы дубликатами.
* `LimitedDropCount="0"`: **неограниченное число выпадений**. В коде `LootGroupsHandler.isDropAllowedForPlayer` условие `drop.getLimitedDropCount() <= 0` возвращает `true` без проверки счетчика базы данных. Предмет может выпадать неограниченное число раз на протяжении всей игры и повторно выбираться в разных слотах одного открытия мешка. Используется для всех базовых расходников, патронов, сырья и строительных материалов.
...
[130–159]  Кулинария, провизия и быт (130: Котелок бродяги, 131: Походный кухонник, 132: Офицерский провиант)
           └ Резерв 133–159: специи, консервированные рационы экспедиций, полевая стерилизация
[160–189]  Агрономия, оранжерея и банк семян (160: Складской мешок семян, 161: Садовый рюкзак, 162: Пенал редких семян)
...
* **Содержимое:**
  ...
  * `hbm:item.bolt:8200` (**Свинцовые болты / метизы радиационной защиты**) | Кол-во: 4–8 | Вес: 50 | `RandomAmount="true"`. *Смысл:* Монтаж свинцовых фартуков и противорадиационных экранов.
...
* **Содержимое:**
  ...
  * `hbm:item.cap_nuka` (**Крышка от Ядер-Колы**) | Кол-во: 4–8 | Вес: 18 | `RandomAmount="true"`. *Смысл:* Базовая довоенная валюта пустошей.
  * `hbm:item.cap_sparkle` (**Крышка от Спаркл-Колы**) | Кол-во: 2–4 | Вес: 12 | `RandomAmount="true"`. *Смысл:* Популярная альтернативная газировка.
  * `hbm:item.cap_korl` (**Крышка от Корла**) | Кол-во: 2–4 | Вес: 12 | `RandomAmount="true"`. *Смысл:* Местная региональная газировка.
  * `hbm:item.cap_fritz` (**Крышка от Фриц-Колы**) | Кол-во: 2–4 | Вес: 12 | `RandomAmount="true"`. *Смысл:* Довоенный импорт.
...
* **Содержимое:**
  ...
  * `hbm:item.plate_stainless` (**Пластина нержавеющей стали**) | Кол-во: 2–4 | Вес: 50 | `RandomAmount="true"`. *Смысл:* Кислотостойкий и антикоррозионный конструкционный прокат.
...
* **Содержимое:**
  ...
  * `hbm:item.plate_stainless` (**Пластина нержавеющей стали**) | Кол-во: 3–6 | Вес: 50 | `RandomAmount="true"`. *Смысл:* Стойкий конструкционный прокат для реакторов и теплообменников.
  * `hbm:item.plate_polymer` (**Полимерная пластина**) | Кол-во: 2–4 | Вес: 45 | `RandomAmount="true"`. *Смысл:* Износостойкие антифрикционные направляющие станков.
  * `hbm:item.bolt:33` (**Болт из дюрастали**) | Кол-во: 6–12 | Вес: 50 | `RandomAmount="true"`. *Смысл:* Высокопрочный крепёж тяжелых станин и турбоагрегатов.
  * `hbm:item.bolt:7400` (**Вольфрамовый болт**) | Кол-во: 4–8 | Вес: 35 | `RandomAmount="true"`. *Смысл:* Жаропрочный крепёж электродуговых и вакуумных печей.
...
* **Содержимое:**
  ...
  * `ProjRed|Transmission:projectred.transmission.framewire:0` (**Провод из красного сплава в каркасе/оболочке**) | Кол-во: 8–16 | Вес: 20 | `RandomAmount="true"`. *Смысл:* Жесткий магистральный провод стойки.
  * `ProjRed|Transmission:projectred.transmission.framewire:17` (**Шина в каркасе/оболочке**) | Кол-во: 4–8 | Вес: 15 | `RandomAmount="true"`. *Смысл:* Жесткая магистральная межблочная шина стойки.
  * `hbm:item.plate_polymer` (**Полимерная диэлектрическая пластина**) | Кол-во: 2–4 | Вес: 45 | `RandomAmount="true"`. *Смысл:* Изоляция шин и монтажные панели серверной стойки.
...
* **Содержимое:**
  ...
  * `harvestcraft:crackerItem` (**Сухие галеты / крекеры**) | Кол-во: 3–6 | Вес: 65 | `RandomAmount="true"`
  * `minecraft:sugar` (**Сахар**) | Кол-во: 3–6 | Вес: 60 | `RandomAmount="true"`
  * `hbm:item.matchstick` (**Походные спички NTM**) | Кол-во: 8–16 | Вес: 65 | `RandomAmount="true"`. *Смысл:* Быстрый розжиг костра.
...
* **Содержимое:**
  ...
  * `harvestcraft:artichokeseedItem` (**Семя артишока**) | Кол-во: 4–8 | Вес: 50 | `RandomAmount="true"`. *Смысл:* Средиземноморский деликатесный овощ высокой ценности.
  * `harvestcraft:okraseedItem` (**Семя бамии / окры**) | Кол-во: 4–8 | Вес: 50 | `RandomAmount="true"`. *Смысл:* Южная стручковая культура, натуральный загуститель для супов (гамбо).
  * `harvestcraft:kiwiseedItem` (**Семя киви**) | Кол-во: 4–8 | Вес: 55 | `RandomAmount="true"`. *Смысл:* Экзотический десертный плод с высоким содержанием витамина C.
  * `harvestcraft:cranberryseedItem` (**Семя клюквы**) | Кол-во: 4–8 | Вес: 60 | `RandomAmount="true"`. *Смысл:* Северная/болотная ягода долгого хранения, витаминные морсы и соусы.
...
* **Содержимое:**
  ...
  * `hbm:item.plate_iron` (**Железные накладные пластины / уголки**) | Кол-во: 2–4 | Вес: 60 | `RandomAmount="true"`. *Смысл:* Мебельная фурнитура и стяжки деревянных конструкций.
...
* **Содержимое:**
  ...
  * `hbm:item.bolt:33` (**Анкерные болты из дюрастали**) | Кол-во: 6–12 | Вес: 45 | `RandomAmount="true"`. *Смысл:* Высокопрочное крепление шахтной крепи при взрывных работах.
...
* **Содержимое:**
  ...
  * `hbm:item.plate_stainless` (**Пластина нержавеющей стали**) | Кол-во: 2–4 | Вес: 55 | `RandomAmount="true"`. *Смысл:* Кислотостойкий конструкционный материал реакторов и колонн.
  * `hbm:item.plate_polymer` (**Полимерная пластина**) | Кол-во: 3–6 | Вес: 60 | `RandomAmount="true"`. *Смысл:* Листовой конструкционный пластик химических емкостей.
  * `hbm:item.bolt:8200` (**Свинцовые болты футеровки**) | Кол-во: 4–8 | Вес: 40 | `RandomAmount="true"`. *Смысл:* Крепление свинцовой футеровки в чанах с агрессивными кислотами.
...
* **Содержимое:**
  ...
  * `hbm:item.canned_conserve:9` (**Пищевой тюбик космонавта**) | Кол-во: 2–4 | Вес: 70 | `RandomAmount="true"`. *Смысл:* Сбалансированный рацион космического пайка.
...
* **Содержимое:**
  * `hbm:item.reacher` (**Вольфрамовые манипуляторные щипцы**) | Кол-во: 1 | Вес: 40 | `LimitedDropCount="1"` | `RandomAmount="false"`. *Смысл:* Безопасная дистанционная перегрузка высокоактивных ТВС и изотопов.
  * `hbm:item.plate_lead` (**Свинцовая защитная пластина**) | Кол-во: 4–8 | Вес: 50 | `RandomAmount="true"`. *Смысл:* Биологическая защита и футеровка пенала.
  * `hbm:item.dosimeter` (**Дозиметр**) | Кол-во: 1 | Вес: 35 | `LimitedDropCount="1"` | `RandomAmount="false"`. *Смысл:* Мониторинг радиационной обстановки.
  * `hbm:item.radx` (**Рад-X**) | Кол-во: 2–4 | Вес: 45 | `RandomAmount="true"`. *Смысл:* Экстренный радиопротектор оператора.
  * `hbm:item.rbmk_pellet_ueu` (**Топливная таблетка РБМК (UEU)**) | Кол-во: 12–24 | Вес: 50 | `RandomAmount="true"`. *Смысл:* Партия таблеток природного/обеднённого урана на твэл.
  * `hbm:item.rbmk_pellet_meu` (**Топливная таблетка РБМК (MEU)**) | Кол-во: 12–24 | Вес: 45 | `RandomAmount="true"`. *Смысл:* Партия таблеток среднеобогащённого урана.
  * `hbm:item.rbmk_pellet_thmeu` (**Топливная таблетка РБМК (Th-MEU)**) | Кол-во: 12–24 | Вес: 40 | `RandomAmount="true"`. *Смысл:* Партия ториево-урановых таблеток.
  * `hbm:item.rbmk_pellet_heu235` (**Топливная таблетка РБМК (HEU-235)**) | Кол-во: 10–20 | Вес: 30 | `RandomAmount="true"`. *Смысл:* Партия таблеток высокообогащённого урана-235.
  * `hbm:item.rbmk_pellet_mox` (**Топливная таблетка РБМК (MOX)**) | Кол-во: 10–20 | Вес: 30 | `RandomAmount="true"`. *Смысл:* Партия смешанного уран-плутониевого оксидного топлива.
  * `hbm:item.rbmk_pellet_lep` (**Топливная таблетка РБМК (LEP)**) | Кол-во: 10–20 | Вес: 25 | `RandomAmount="true"`. *Смысл:* Партия низкообогащённого плутониевого топлива.
  * `hbm:item.rbmk_pellet_hecm` (**Топливная таблетка РБМК (HECm)**) | Кол-во: 8–16 | Вес: 15 | `RandomAmount="true"`. *Смысл:* Партия высокообогащённого кюриевого топлива.
  ...
  * `hbm:item.watz_pellet:5` (**Топливная микросфера Ватц (MEU)**) | Кол-во: 16–32 | Вес: 40 | `RandomAmount="true"`. *Смысл:* Партия микротвэлов урана реактора Ватц.
  * `hbm:item.watz_pellet:8` (**Поглощающая микросфера Ватц (Бор)**) | Кол-во: 12–24 | Вес: 35 | `RandomAmount="true"`. *Смысл:* Борный замедлитель/поглотитель.
  * `hbm:item.watz_pellet:9` (**Воспроизводящая микросфера Ватц (DU)**) | Кол-во: 12–24 | Вес: 35 | `RandomAmount="true"`. *Смысл:* Обеднённый уран для бридинга плутония.
  * `hbm:item.watz_pellet:6` (**Топливная микросфера Ватц (MEP)**) | Кол-во: 12–24 | Вес: 25 | `RandomAmount="true"`. *Смысл:* Плутониевый микротвэл Ватц.
  * `hbm:item.watz_pellet:14` (**Топливная микросфера Ватц (AMF)**) | Кол-во: 8–16 | Вес: 15 | `RandomAmount="true"`. *Смысл:* Америциевый микротвэл Ватц.
  * `hbm:item.watz_pellet:16` (**Топливная микросфера Ватц (CMF)**) | Кол-во: 8–16 | Вес: 10 | `RandomAmount="true"`. *Смысл:* Кюриевый микротвэл Ватц.
  * `hbm:item.watz_pellet:17` (**Топливная микросфера Ватц (Берклий-247)**) | Кол-во: 6–12 | Вес: 5 | `RandomAmount="true"`. *Смысл:* Редкий берклиевый микротвэл.
  ...
  * `hbm:item.rod:8` (**Стержень из урана-238**) | Кол-во: 2–4 | Вес: 40 | `RandomAmount="true"`. *Смысл:* Воспроизводящий стержень для облучения нейтронами.
  ...
  * `hbm:item.rod:0` (**Литиевый стержень**) | Кол-во: 2–4 | Вес: 30 | `RandomAmount="true"`. *Смысл:* Наработка трития при нейтронном облучении.
  * `hbm:item.rod:2` (**Кобальтовый стержень**) | Кол-во: 2–4 | Вес: 25 | `RandomAmount="true"`. *Смысл:* Наработка радиоактивного кобальта-60.
  ...
  * `hbm:item.cell_deuterium` (**Пробирка с дейтерием**) | Кол-во: 4–8 | Вес: 35 | `RandomAmount="true"`. *Смысл:* Изотоп для реакций термоядерного синтеза.
  * `hbm:item.cell_tritium` (**Пробирка с тритием**) | Кол-во: 4–8 | Вес: 35 | `RandomAmount="true"`. *Смысл:* Тритиевый компонент дейтерий-тритиевой плазмы.
  * `hbm:item.icf_pellet` (**Мишень термоядерного синтеза (ICF Pellet)**) | Кол-во: 4–8 | Вес: 20 | `RandomAmount="true"`. *Смысл:* Мишень для лазерного термоядерного поджига.
...
* **Содержимое:**
  ...
  * `hbm:item.wire_dense:31` (**Плотный провод красной меди**) | Кол-во: 2–4 | Вес: 35 | `RandomAmount="true"`
  * `hbm:item.wire_dense:7900` (**Плотный золотой провод**) | Кол-во: 2–4 | Вес: 35 | `RandomAmount="true"`
  * `hbm:item.wire_dense:2200` (**Плотный титановый провод**) | Кол-во: 2–4 | Вес: 35 | `RandomAmount="true"`
  * `hbm:item.wire_dense:7400` (**Плотный вольфрамовый провод**) | Кол-во: 2–4 | Вес: 35 | `RandomAmount="true"`
  ...
  * `appliedenergistics2:item.ItemMultiPart:120` (**Кабельный якорь AE2**) | Кол-во: 8–16 | Вес: 35 | `RandomAmount="true"`. *Смысл:* Фиксация кабельных трасс бортовой сети шаттла.
  * `appliedenergistics2:tile.BlockQuartzTorch` (**Кварцевый факел AE2**) | Кол-во: 4–8 | Вес: 35 | `RandomAmount="true"`. *Смысл:* Автономные маркерные огни и кварцевое освещение отсеков.
  * `appliedenergistics2:item.ItemMultiPart:140` (**Переключающий кабель / Toggle Bus AE2**) | Кол-во: 4–8 | Вес: 25 | `RandomAmount="true"`. *Смысл:* Коммутация бортовых субсетей и аварийных контуров.
  ...
  * `hbm:item.bolt:7400` (**Вольфрамовые жаропрочные болты**) | Кол-во: 4–8 | Вес: 45 | `RandomAmount="true"`. *Смысл:* Крепёж противометеоритной обшивки и теплозащитных щитов сопел.
...
2. **Справедливое вознаграждение за смертельный риск:**
   ...
   * В запертых сейфах (`POOL_EXPENSIVE`) гарантирована возможность получения элитных сумок Tier 3 (`Meta 12`, `102`, `192`, `281`) с вероятностью **~51%** на сейф.
...
| **311** | Транспортный пенал ядерного топлива | 3 | 3 | 1 (моно-поставка) | false | 63 |
...
* **Возможные решения (бэклог / техдолг):**
  1. *Патч / хук в `ntnhcore`:* по событию `FMLServerStoppedEvent` сбрасывать приватное поле `_mPersistedDB = null` через рефлексию, чтобы каждый заход в мир принудительно пересоздавал хранилище под текущий сейв.
  2. *Админ-команда:* реализовать команду сброса лимитов (например, `/lbag resetlimits [player]`).
  3. *Геймдизайн:* снять жесткие лимиты `LimitedDropCount="0"` со всех бытовых расходников и инструментов (ножи, ключи, посуда, мыло), оставив их только на уникальных сюжетных наградах.
