...
1. **Никакого абстрактного мусора:**
   Каждый контейнер — это аутентичная личная вещь, ЗИП или инструментальная сумка (*«Сумка электрика»*, *«Патронный цинк»*, *«Носимый аварийный запас»*). Вторичный лут знакомит игрока с механиками модов сборки (ModernMarkings, BiblioCraft, Storage Drawers, Computronics).
...
3. **Трёхступенчатая шкала сохранности и объёма дропа:**
   ...
   * 🥉 **Tier 1: Потрёпанный / Вскрытый** (`2–4 типа/слота предметов`, отдельные профили — `1–2` или `1–3`). Найден на поверхности и в открытых заброшках. Содержит базовый инструмент, расходники и тематический лом. Флаг `CombineTrashGroup="false"` установлен для всех контейнеров: каждый мешок имеет собственный специализированный вторичный лом (ветошь, обрезки проводов, гильзы), что исключает засорение дропа ванильными палками/булыжником и отключает неактуальные тултипы зачарования на Удачу (`AllowFortuneBags=false`).
   * 🥈 **Tier 2: Штатный / Производственный** (`2–4 типа/слота предметов`, расширенный — `3–5` слотов, моно-партии — `1` слот). Шкафчики заводских цехов, бытовки, лаборатории. Профильный инструмент, приборы и качественные расходники (`CombineTrashGroup="false"`).
   * 🥇 **Tier 3: Опечатанный / Военный / Заводской** (`2–5 типов/слотов предметов`, спец-наборы — `1–3` или `2–4` слота). Сейфы бункеров NTM, закрытые лаборатории, место крушения челнока. Высокотехнологичное оборудование и чистые материалы без постороннего мусора (`CombineTrashGroup="false"`).
...
| **132** | Офицерский провиант | 3 | 2 | 3–5 | false | 33 |
| **160** | Складской мешок семян | 1 | 0 | 1 (база 81–100) | false | 25 |
| **161** | Садовый рюкзак | 2 | 1 | 3–5 | false | 32 |
| **162** | Спецконтейнер семян | 3 | 2 | 2–3 | false | 11 |
| **190** | Мешок плотника | 1 | 0 | 2–4 | false | 27 |
| **191** | Сумка маляра | 2 | 1 | 2–4 | false | 34 |
| **192** | Кейс проектировщика | 3 | 2 | 3–5 | false | 26 |
| **220** | Планшет разведчика | 1 | 0 | 2–4 | false | 23 |
| **221** | Геодезический набор | 2 | 1 | 2–4 | false | 18 |
| **222** | Ящик шахтной взрывотехники | 3 | 2 | 3–6 | false | 24 |
| **250** | Сумка сигнальщика | 1 | 0 | 2–4 | false | 20 |
| **251** | Радиоукладка | 2 | 1 | 2–4 | false | 20 |
